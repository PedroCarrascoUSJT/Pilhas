public class PilhaChar {
   ...
   static boolean palindromo(String s) {
        ...
        String fraseInvertida = inverteFrase(s);

        for(int i = 0;i < s.length();i++){
            p.push(fraseInvertida.charAt(i));
        }

        for(int i = 0;i < s.length();i++){
            char fraseChar = p.pop();
            char fraseInevertidaChar = fraseInvertida.charAt(i);
            if(fraseChar != fraseInevertidaChar)
                return false;
        }
   ...
   static String inverteFrase(String s) {
      PilhaChar p = new PilhaChar(s.length());
    ...
    public static void main(String[] args) throws Exception {
      ...
      for(int i=0;i<palavra.length();i++){
		...
		System.out.println("esta vazia? "+p2.isempty());
		System.out.println("esta cheia? "+p2.isfull());
        boolean resp = palindromo("arara");
		System.out.println("palíndromo? "+resp);
    }
}
